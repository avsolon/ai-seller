"""Simplified script to import knowledge base data with proper encoding."""
...
def slugify(text: str) -> str:
    ...
    text = re.sub(r'[_]+$', '', text)  # Remove trailing underscores
...
"""
        ...
        for product in products:
            ...
            safe_name = f"{product_id}_{slugify(name)}"
...
def create_faq_from_products(products_data: List[Dict[str, Any]], output_dir: Path) -> None:
    ...
    led_lenses = [p for p in products_data if "линз" in p.get("category", "").lower()]
    if led_lenses:
        faq_entries.append({
            "id": "faq_led_001",
        ...
        faq_entries.append({
            ...
            "answer": "Цветовая температура измеряется в Кельвинах. 4300K - теплый белый свет, 5000K - нейтральный белый, 5500K-6000K - холодный белый с голубоватым оттенком. Для городского использования часто выбирают 5000-5500K.",
            "category": "technical",
    ...
    xenon_lamps = [p for p in products_data if "ксенон" in p.get("category", "").lower()]
    if xenon_lamps:
        faq_entries.append({
            "id": "faq_xenon_001",
