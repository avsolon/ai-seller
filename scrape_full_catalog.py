#!/usr/bin/env python3
"""
Full catalog scraper for faraline.ru
...
def get_product_urls_from_page(page_url: str) -> List[str]:
    ...
    try:
        ...
        # Try to find product links by class name first
        product_links = soup.find_all('a', class_=lambda x: x and 'product' in x.lower())
        ...
        # Also try to find all links with product in href
...
def parse_characteristics_from_text(text: str) -> Dict:
    ...
    # Common model patterns: ORIONLIGHT, PL3-24, etc.
...
def merge_characteristics(from_name: Dict, from_desc: Dict, from_table: Dict) -> Dict:
    ...
    merged = {
        "тип": from_name.get("тип", "") or from_table.get("Тип", "") or from_table.get("тип", ""),
        "модель": from_name.get("модель", "") or from_table.get("Модель", "") or from_table.get("модель", ""),
        "размер": from_name.get("размер", "") or from_table.get("Размер", "") or from_table.get("размер", ""),
        "напряжение": from_name.get("напряжение", "") or from_table.get("Напряжение", "") or from_table.get("напряжение", ""),
        "цветовая_температура": from_name.get("цветовая_температура", "") or from_table.get("Цветовая температура", "") or from_table.get("цветовая температура", ""),
        "мощность": from_name.get("мощность", "") or from_table.get("Мощность", "") or from_table.get("мощность", ""),
        "комплектация": from_name.get("комплектация", "") or from_table.get("Комплектация", "") or from_table.get("комплектация", ""),
        "особенности": from_name.get("особенности", "") or from_table.get("Особенности", "") or from_table.get("особенности", ""),
    }
    return merged
...
def scrape_product_page(product_url: str) -> Optional[Dict]:
    ...
    try:
        ...
        if category != "Unknown":
            ...
            product_data['category'] = extract_category_from_url(product_url)
        ...
        # Parse characteristics from name and description
